...
class H5TrackAssemble extends BaseEventAssemble {
    ...
    public H5TrackAssemble(InternalConfigOptions internalConfigs) {
    ...
    private void appendNativeProperty(EventType eventType, TrackEvent trackEvent, SAContextManager contextManager) throws JSONException {
        ...
        if (libObject != null) {
            libObject.put("$app_version", AppInfoUtils.getAppVersionName(mInternalConfigs.context));
            //update lib $app_version from super properties
            JSONObject superProperties = contextManager.getSuperProperties().get();
        ...
        SADataHelper.addCarrier(mInternalConfigs.context, trackEvent.getProperties());
    ...
    private void appendPropertyPlugin(EventType eventType, TrackEvent trackEvent, SensorsDataAPI sensorsDataAPI) throws JSONException {
        ...
        filter.setEventJson(SAPropertyFilter.LIB, new JSONObject(libProperties.toString()));
        filter.setProperties(pluginJSON);
        filter.setType(eventType);
        // 获取预置属性到 filter 中
        sensorsDataAPI.getSAContextManager().getPluginManager().propertiesHandler(filter);
        Iterator<String> iterator = pluginJSON.keys();
        while (iterator.hasNext()) {
            ...
            if (!TextUtils.isEmpty(key)) {
                ...
                trackEvent.getProperties().put(key, pluginJSON.opt(key));
            }
        }
    }
